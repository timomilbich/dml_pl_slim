    ...
    def compute_standard(self, opt, model, dataloader, evaltypes, device, **kwargs):
        ...
        with torch.no_grad():
            ...
            for idx,inp in enumerate(final_iter):
                ...
                if 'pre_feat' not in feature_colls: feature_colls['pre_feat'] = []
                feature_colls['pre_feat'].extend(torch.nn.functional.normalize(out_dict['avg_features'].view(len(input_img),-1), dim=-1).cpu().detach().numpy().tolist())

                ### Include embeddings of all output features
                if extra_out is not None:
                    if 'large_embeds' not in feature_colls: feature_colls['large_embeds'] = []
                    feature_colls['large_embeds'].extend(extra_out.cpu().detach().numpy().tolist())

                for evaltype in evaltypes:
                    if 'Combined' not in evaltype and 'Sum' not in evaltype:
                        feature_colls[evaltype].extend(out_dict[evaltype].cpu().detach().numpy().tolist())
        ...
        if extra_out is not None:
            evaltypes.append('large_embeds')
        evaltypes.append('pre_feat')
            ...
            with torch.no_grad():
                ...
                for evaltype in evaltypes:
                    ...
                    if 'kmeans' in self.requires:
                        ...
                        cluster_idx = faiss.IndexFlatL2(stackset.shape[-1])
                        kmeans            = faiss.Clustering(stackset.shape[-1], n_classes)
                        kmeans.niter = 20
                        kmeans.min_points_per_centroid = 1
                        kmeans.max_points_per_centroid = 1000000000
                        ### Train Kmeans
                        kmeans.train(stackset, cluster_idx)
